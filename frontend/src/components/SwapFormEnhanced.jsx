...
import { ArrowDown, RefreshCw, Info, Zap, Settings } from 'lucide-react';
...
const SwapFormEnhanced = ({ chainId, walletAddress }) => {
  ...
  useEffect(() => {
    if (sellToken) {
      const coinId = getCoinGeckoId(sellToken);
  ...
  const fetchQuote = async () => {
    ...
    try {
      ...
      const outputAmount = ethers.formatUnits(response.data.buyAmount, getTokenDecimals(buyToken));
      const expectedOutput = parseFloat(sellAmount); // Simplified, should use actual price
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      try {
        ...
        if (referrer) {
          await axios.post(`${API}/referrals`, {
            ...
            chain: chainNames[chainId],
            token_pair: `${getTokenSymbol(sellToken)}-${getTokenSymbol(buyToken)}`,
      ...
      saveSwapToHistory({
        ...
        amountIn: sellAmount,
        amountOut: ethers.formatUnits(quote.buyAmount, getTokenDecimals(buyToken)),
        txHash: txHash
      });
            ...
            <div className="flex-1 space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  1 {getTokenSymbol(sellToken)} = {(parseFloat(ethers.formatUnits(quote.buyAmount, getTokenDecimals(buyToken))) / parseFloat(sellAmount)).toFixed(6)} {getTokenSymbol(buyToken)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Platform Fee (0.2%):</span>
                <span className="font-medium">
                  {(parseFloat(ethers.formatUnits(quote.buyAmount, getTokenDecimals(buyToken))) * 0.002).toFixed(6)} {getTokenSymbol(buyToken)}
                </span>
              </div>
              <div className="flex justify-between">
